...
class OpenMiChroM:
    ...
    ##initialize the OpenMiChroM set##
    def __init__(
    ...
    def setup(self, platform="CUDA", GPU="default", integrator="langevin", errorTol=None, precision="mixed"):
        ...
        self.bondsForException2 = []
        self.mm = openmm
        self.conlen = 1. * units.meter * 1e-9 * self.length_scale
    ...
    def load(self, filename,  
        ...
        """
        Load the positions into the openMM context
    ...
    def setData(self, data, random_offset = 1e-5):
        ...
        self.data = units.Quantity(data, units.meter * 1e-9)
    ...
    def getData(self):
        ...
        """
        return np.asarray(self.data / (units.meter * 1e-9), dtype=np.float32)
    ...
    def addSphericalConfinementLJ(self,
                ...
                density=0.1):  # target density, measured in particles
                                # per cubic nanometer (bond size is 1 nm)
        """
        Spherical Confinement using the repulsive term of lennard-Jones
        ...
        spherForce = self.mm.CustomExternalForce("(4 * e * ((s/r)^12 - (s/r)^6) + e) * step(cut - r);"
        ...
        spherForce.addGlobalParameter('e', 1.0)
        spherForce.addGlobalParameter('s', 1.0)
        spherForce.addGlobalParameter("cut", 2.**(1./6.))
    ...
    def _initHarmonicBondForce(self):
        "Internal, inits harmonic force for polymer and non-polymer bonds"
    ...
    def addFENEBonds(self, k=30):
        ...
        for start, end, isRing in self.chains:
            ...
            if isRing:
                self.addBond(start, end - 1, distance=1, bondType="FENE", k=k)
    ...
    def _initFENEBond(self, k=30):
        ...
        if "FENEBond" not in list(self.forceDict.keys()):
            force = ("- 0.5 * k * r0 * r0 * log(1-(r/r0)*(r/r0)) * step(r0 - r) + (4 * e * ((s/r)^12 - (s/r)^6) + e) * step(cut - r)")
    ...
    def addAngles(self, k=2.0):
        ...
        for start, end, isRing in self.chains:
            for j in range(start + 1, end):
                ...
                self.bondsForException2.append((j - 1, j + 1)) #exceptions for 2 neighhood
    ...
    def addRepulsiveSoftCore(self, Ecut=4.0):
        """Non-bonded pairs experience a soft-core repulsive interaction
        ...
        repul_energy = ("select(delta(r), 0, energy);"
                       "energy = LJ * step(r - r_0) * step(CutOff - r)"
    ...
    def addTypetoType(self, mi=3.22, rc = 1.78 ):
        ...
        Important see the list of types first
        default: values for human genome
        ...
        energy = "mapType(t1,t2)*0.5*(1. + tanh(mi*(rc - r)))"
    ...
    def addTypes_forTrainner(self, mi=3.22, rc = 1.78, lambdas=[0,0,0]):
        """
        Type to type interactions using for trainner new  parameters
        for AA AB BB interactions
    ...
    def IC_forTrainner(self, mi=3.22, rc = 1.78, dcutl=3, dcutupper=200, lambdas=None):
        """
        Ideal chromosome interaction using for trainer parameters
    ...
    def _applyForces(self):
        ...
        exc2 = self.bondsForException2
        print("Number of exceptions:\nFirst neighbor= {}\nSecond neighbor= {}".format(len(exc), len(exc2)))
        ...
        if len(exc) > 0:
            ...
            exc2 = np.array(exc2)
            exc2 = np.sort(exc2, axis=1)
            exc2 = [tuple(i) for i in exc2]
            exc2 = list(set(exc2))  # only unique pairs are left

        for i in list(self.forceDict.keys()):  # Adding exceptions
            force = self.forceDict[i]
            if hasattr(force, "addException"):
                print('Add exceptions for {0} force (1 neighbor)'.format(i))
                for pair in exc:
                ...
                print('Add exclusions for {0} force (1 neighbor)'.format(i))
                for pair in exc:
    ...
    def initStorage(self, filename, mode="w"):
        ...
        for k, chain in zip(range(len(self.chains)),self.chains):
            ...
            if hasattr(self, "loopPosition"):
                self.storage[k]['loops'] = self.loopPosition[chain[0]:chain[1]+1]
...
################################################################################
    ...
    def doBlock(self, steps=None, increment=True, num=None, reinitialize=True, maxIter=0, output=True, checkFunctions=[]):
        ...
        for attempt in range(15):
            ...
            b = time.time()
            coords = self.state.getPositions(asNumpy=True)
            newcoords = coords / (units.meter * 1e-9)
            ...
            if ((np.isnan(newcoords).any()) or (eK > self.eKcritical) or
                ...
                #mdist = np.diagonal(distance.cdist(self.getData(),self.getData(), 'euclidean'),offset=1)
                #print("top 5 short distances 1: ",  np.sort(mdist)[:5])
                #mdist = np.diagonal(distance.cdist(self.getData(),self.getData(), 'euclidean'),offset=2)
                #print("top 5 short distances 2: ",  np.sort(mdist)[:5])
                #mdist = np.diagonal(distance.cdist(self.getData(),self.getData(), 'euclidean'),offset=3)
                #print("top 5 short distances 3: ",  np.sort(mdist)[:5])
                #mdist = np.diagonal(distance.cdist(self.getData(),self.getData(), 'euclidean'),offset=4)
                #print("top 5 short distances 4: ",  np.sort(mdist)[:5])
                #mdist = np.diagonal(distance.cdist(self.getData(),self.getData(), 'euclidean'),offset=5)
                #print("top 5 short distances 5: ", np.sort(mdist)[:5])
                ...
                if (self.integrator_type.lower() == 'variablelangevin'
                    ...
                    print('dt=%.1lffs' % (dt / (units.second * 1e-15)), end=' ')
                    mass = self.system.getParticleMass(1)
                    dx = (units.sqrt(2.0 * eK * self.kT / mass) * dt)
                    print('dx=%.2lfpm' % (dx / (units.meter * 1e-9) * 1000.0), end=' ')
