...
def test_inspect_structural_file_cli_url_list_json_output(tmp_path):
    cndb_path = tmp_path / "simple.cndb"
    ndb_path = tmp_path / "simple.ndb"
    urls_path = tmp_path / "sources.txt"
    ...
    urls_path.write_text(f"{cndb_path}\n{ndb_path}\n", encoding="utf-8")
    ...
    assert [entry["file_type"] for entry in payload] == ["cndb", "ndb"]
    assert payload[0]["layout"] == "openmichrom-simple-cndb"
    assert payload[1]["layout"] == "text-ndb"
