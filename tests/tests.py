...
from OpenMiChroM.Optimization import (
    FullTraining,
...
class testMichrom:
    ...
    def runDefault(self):
        ...
        myChrom = sim.initStructure(
            mode="spring",
            ChromSeq="chr10/chr10_beads.txt",
        )
        sim.loadStructure(myChrom, center=True)
        sim.addFENEBonds()
        sim.addAngles()
    ...
    def testCustomMiChroM(self):
        b = CustomMiChroMTraining(
            ChromSeq=sys.path[0] + "/training/seq_c18_10"
        )
        ...
        ic = b.getLamb_types(
            exp_map=sys.path[0] + "/training/c18_10.dense"
        )
        print("Finished")
    ...
    def testCndbTools(self):
        ...
        sampleA1 = cndbt.xyz(
            frames=[1, 100, 1], beadSelection=traj.dictChromSeq["A1"]
        )
        sampleB1 = cndbt.xyz(
            frames=[1, 100, 1], beadSelection=traj.dictChromSeq["B1"]
        )
        ...
        print("Computing RDP...")
        xa1, rdp_a1 = cndbt.compute_RDP(
            sampleA1, radius=20.0, bins=200
        )
        xb1, rdp_b1 = cndbt.compute_RDP(
            sampleB1, radius=20.0, bins=200
        )

        print("Computing Chirality...")
        psi = cndbt.compute_Chirality(
            cndbt.xyz(frames=[1, 100, 1]), 4
        )
    ...
    def testAdamTraining(self):
        ...
        with h5py.File("AdamTraining/input/Pi_0.h5", "r") as hf:
            opt.Pi += hf["Pi"][:]
            opt.NFrames += int(
                np.loadtxt("AdamTraining/input/Nframes_0")
            )
